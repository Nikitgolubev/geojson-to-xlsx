  ...
  function showHelp() {
    ...
    body.innerHTML = `
      ...
      <ul>
        ...
        <li><b>Проверка адреса</b> — введите адрес (подсказки по мере ввода), программа
          геокодирует его и проверяет, в какие сохранённые зоны попадают координаты.
          Результат зелёным (входит, со списком зон) или красным (не входит) + карта и
          журнал проверки. Кнопка «Очистить результаты».</li>
        <li><b>Создание полигона</b> — нарисуйте полигон кликами по карте. Поле «Найти
          место» позиционирует карту по адресу перед рисованием. «Отменить точку» /
          «Начать заново» / «Сохранить» (с запросом имени). Готовый полигон попадает в
          «Зоны» (без города) и скачивается файлом .geojson в «Загрузки».</li>
      ...
      <ul>
        ...
        <li><b>Помощь → Проверить обновления</b> — поиск новой версии на GitHub; перед
          установкой обновления программа автоматически сохраняет резервную копию данных
          (города и зоны) в папку «Загрузки».</li>
