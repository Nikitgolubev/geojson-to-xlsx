...
> 🖥️ **ЭТО ДЕСКТОП-РЕПОЗИТОРИЙ `geojson-to-xlsx` (продукт `polygons`).**
> Веб-версия (клиент-сервер) разрабатывается ОТДЕЛЬНО в репозитории `geojson_web` и сюда не
> вливается. Веб-код в этот репозиторий не добавлять; десктоп продолжает свою линию `v0.9.x`.
...
electron/github-sync.js — computeHash/buildManifest/decideVerdict (чистый; синк данных, v0.9.0)
...
src/views/sync.js     — «Обновление данных»: синхронизация с GitHub (v0.9.0)
src/views/log.js      — журнал действий
src/vendor/leaflet/   — локальная копия Leaflet (js/css/images, без CDN)
backups/              — синк-копия данных в GitHub: manifest.json + latest.json (v0.9.0)
```
...
- `sync.getToken() / setToken(t) / ping() / check() / push() / pull()` — синхронизация с
  GitHub (v0.9.0). `ping` — лампочка доступа; `check` — вердикт актуальности; `push`/`pull`
  (pull = полная замена `db.data.replaceAll` + бэкап в «Загрузки»)
- `api.on(channel, cb)` — события main→renderer, белый список: `menu:help`, `data:changed`,
  `menu:set-theme`, `menu:feedback`, `menu:bug`, `sync:progress`
...
- **v0.9.0 — вкладка «Обновление данных»** (`src/views/sync.js`): синхронизация
  городов+зон с GitHub (папка `backups/`). Push (нужен PAT-токен) → `backups/latest.json`
  (полный снимок) + `backups/manifest.json` (мета: version,savedAt,dataHash,counts) через
  Contents API; версия данных = инкремент + история git. Pull = **полная замена**
  (`db.data.replaceAll`) + авто-бэкап в «Загрузки». «Проверить актуальность» сравнивает
  `computeHash(exportAll)` с `manifest.dataHash` → вердикт (`decideVerdict`:
  ok/local-ahead/remote-ahead/diverged/empty). Токен — видимое поле, хранится в
  `userData/github-token.json` (вне репо). **Лампочка** доступа — `sync.ping` (GET репо с
  токеном). Прогресс — события `sync:progress`. Состояние синка —
  `userData/sync-state.json` (для направления расхождения). Чистая логика —
  `electron/github-sync.js`; тесты `test:sync` (логика) и `test:sync-ui` (вкладка).
  **Авторизация обязательна для записи** (desktop-приложение не пишет в репо без токена).
