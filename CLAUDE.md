...
`test:func`, `test:badge` (через Electron в изолированной временной БД, env GZ_*).
...
- Иконка приложения пока не задана (electron-builder берёт дефолтную) — добавить
  `build/icon.ico` при необходимости.
